...
def obs_seq_header(file, debug_header=False):
    ...
    num_ob_kinds = int(fi.readline())  # Read(int) "number of observation types" 
...
def read_obs_seq(fhead, return_DF = False, return_XR = False, time_from_1800=None):
    ...
    if time_from_1800:
        anal_dtime           = obs_seq_file_DT(fhead.filename)
        print((anal_dtime.strftime(time_format)))
    ...
    for m, n in enumerate(idx_obs):
        ...
        stuff             = stuff.split()
        obs_seq.number[i] = int(stuff[1]) - 1
        ...
        previous         = int(stuff[0])
        next             = int(stuff[1])
        cov_group        = int(stuff[2])
        ...
        if len(stuff) == 4:
            obs_seq.vert_coord[i] = int(stuff[3])
            j += 2
        else:
            obs_seq.vert_coord[i] = int(fi[j+1])
            j += 3
...
# Command line interface for DART_cc
    ...
    time_stamp = obs_seq_file_DT(files[0])
...
#   Add attributes to the files
    ...
    for key in list(file_header0.obs_type.keys()):
